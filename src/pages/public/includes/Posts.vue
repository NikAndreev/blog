<template>
  <v-card max-width="800" class="ml-auto mr-auto" :loading="isPending">
    ...
    <v-card-text>
      <v-list v-if="posts.length" lines="two">
        <v-list-item v-for="post in posts" :key="post.id" :value="post.id">
          <v-list-item-title>
            <router-link
              :to="`/${post.id}`"
      ...
      <div v-else class="text-h5">Постов пока нет</div>
    </v-card-text>
  </v-card>
</template>
